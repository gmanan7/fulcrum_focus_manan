 ...
 * - Calls supabase.rpc('update_task_due_date', { p_task_id, p_new_due_date, p_reason })
...
describe('changeDueDateMutation', () => {
  ...
  it('calls update_task_due_date RPC with correct parameters', async () => {
    await runChangeDueDate(supabase, { taskId: 't1', newDueDate: '2026-06-15', reason: 'slip' });
  ...
  it('does NOT write directly to tasks or task_updates tables', async () => {
    await runChangeDueDate(supabase, { taskId: 't2', newDueDate: '2026-07-01', reason: 'r' });
    expect(supabase.from).not.toHaveBeenCalled();
  });

  it('surfaces RPC errors', async () => {
    supabase.rpc.mockResolvedValueOnce({ error: new Error('only creator/admin/leader can change due date') });
    await expect(
      runChangeDueDate(supabase, { taskId: 't3', newDueDate: '2026-07-01', reason: 'r' }),
