 ...
 * Direct supabase.from('tasks').update for these fields is no longer allowed.
 */
...
async function runEditTaskFields(
  ...
  const { error } = await supabase.rpc('update_task_fields', {
...
describe('Edit Task save → update_task_fields RPC', () => {
  ...
  it('calls update_task_fields with the correct payload', async () => {
    await runEditTaskFields(supabase, {
      taskId: 't1', title: 'New', description: 'D', ownerId: 'u1', priority: 'high', departmentId: 'd1',
    });
    expect(supabase.rpc).toHaveBeenCalledWith('update_task_fields', {
      p_task_id: 't1',
      p_title: 'New',
      p_description: 'D',
      p_owner_id: 'u1',
  ...
  it('surfaces RPC errors (toast-able message)', async () => {
    supabase.rpc.mockResolvedValueOnce({ error: new Error('only creator/admin/HOD/leader can edit') });
    await expect(
      runEditTaskFields(supabase, { taskId: 't', title: 'x', description: null, ownerId: 'u', priority: 'low', departmentId: 'd' }),
...
describe('Change Due Date → update_task_due_date RPC only', () => {
  ...
  it('calls update_task_due_date with reason', async () => {
    await runChangeDueDate(supabase, { taskId: 't', newDueDate: '2026-07-01', reason: 'slip' });
    expect(supabase.rpc).toHaveBeenCalledWith('update_task_due_date', {
      p_task_id: 't', p_new_due_date: '2026-07-01', p_reason: 'slip',
    });
  });

  it('does NOT touch tasks or task_due_date_history directly', async () => {
    await runChangeDueDate(supabase, { taskId: 't', newDueDate: '2026-07-01', reason: 'r' });
    expect(supabase.from).not.toHaveBeenCalled();
  });
});
