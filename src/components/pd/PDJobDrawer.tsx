...
export function PDJobDrawer({ job, open, canManage, onClose, onNavigateToJob }: Props) {
  ...
  const postComment = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('pd_job_comments').insert({
...
function SpawnModal({
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase.rpc('spawn_pd_job_from', {
...
function EditModal({
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('pd_jobs').update({
