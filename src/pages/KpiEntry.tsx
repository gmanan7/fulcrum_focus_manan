...
function useUserDepartments() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: deptIds } = await supabase.rpc('get_user_departments', { p_user_id: user!.id });
      if (!deptIds || deptIds.length === 0) return [];
      const ids = deptIds.map((d: any) => d.department_id);
      const { data, error } = await supabase.from('department').select('id, name, code').in('id', ids).eq('is_active', true).order('display_order');
      if (error) throw error;
      return data;
    },
    enabled: !!user,
...
function TrackerKpiSection({ kpiId, kpiName, departmentId, reportingDate, hideCompleted }: {
  ...
  const addItemMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from('project_tracker_items').insert({
        kpi_id: kpiId,
  ...
  const statusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: ProjectItemStatus }) => {
      const { error } = await supabase.from('project_tracker_items').update({ status, updated_at: new Date().toISOString() }).eq('id', id);
      if (error) throw error;
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('project_tracker_items').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
...
function TrackerItemCard({ item, reportingDate, onStatusChange, onDelete }: {
  ...
  const addUpdateMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from('project_item_stage_updates').insert({
        item_id: item.id,
