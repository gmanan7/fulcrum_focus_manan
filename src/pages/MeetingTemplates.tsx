...
export default function MeetingTemplates() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('meeting_templates').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
...
function ManageInviteesSheet({ templateId, onClose }: { templateId: string | null; onClose: () => void }) {
  ...
  const { data: allUsers } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('profiles').select('id, full_name').eq('is_active', true).order('full_name');
  ...
  const addMutation = useMutation({
    mutationFn: async (userId: string) => {
      const { error } = await supabase.from('meeting_template_invitees').insert({
  ...
  const removeMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('meeting_template_invitees').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['template-invitees', templateId] });
  ...
  const toggleMandatory = useMutation({
    mutationFn: async ({ id, val }: { id: string; val: boolean }) => {
      const { error } = await supabase.from('meeting_template_invitees').update({ is_mandatory: val }).eq('id', id);
      if (error) throw error;
    },
