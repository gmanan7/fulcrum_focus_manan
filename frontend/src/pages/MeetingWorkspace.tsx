...
export default function MeetingWorkspace() {
  ...
  const handleEnd = async () => {
    ...
    const { data: redEntries } = await DB
      .from('kpi_entries')
      .select('id, kpi_id')
...
function KpiSnapshotTab({ meeting }: { meeting: any }) {
  ...
  const { data: departments } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('department').select('id, name, code').eq('is_active', true).order('display_order');
      return data || [];
    },
  });
  ...
  const { data: kpis } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('kpi_master').select('*').eq('is_active', true).in('kpi_type', ['numeric', 'descriptive']).order('display_order');
      return data || [];
    },
  });
  ...
  const { data: entries } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('kpi_entries').select('*').eq('reporting_date', kpiDate);
      return data || [];
    },
  });
  ...
  const { data: tasks } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await DB.from('tasks').select('origin_kpi_entry_id').in('origin_kpi_entry_id', entryIds);
      return data || [];
    },
    enabled: !!entries,
  });
...
function CreateTaskFromKpiModal({ open, onOpenChange, meetingId, kpiEntry, kpi }: {
  ...
  const { data: deptUsers } = useQuery({
    ...
    queryFn: async () => {
      const { data: uds } = await DB.from('user_departments').select('user_id').eq('department_id', kpi.department_id);
      if (!uds?.length) return [];
      const { data } = await DB.from('profiles').select('id, full_name').in('id', uds.map((u) => u.user_id)).eq('is_active', true);
      return data || [];
    },
    enabled: open,
  });
...
function AttendanceTab({ meeting, readOnly }: { meeting: any; readOnly: boolean }) {
  ...
  const { data: attendance } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('meeting_attendance').select('*').eq('meeting_id', meeting.id);
      return data || [];
    },
  });
  ...
  const { data: allUsers } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('profiles').select('id, full_name, email').eq('is_active', true).order('full_name');
      return data || [];
    },
  });
  ...
  const addInviteeMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await DB.from('meeting_invitees').insert(payload);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const markAttendanceMutation = useMutation({
    mutationFn: async ({ inviteeId, status, remarks }: { inviteeId: string; status: AttendanceStatus; remarks?: string }) => {
      ...
      if (existing) {
        const { error } = await DB.from('meeting_attendance').update({ status, remarks: autoRemarks || null }).eq('id', existing.id);
        if (error) throw error;
      } else {
...
function NotesTab({ meeting, readOnly, onUnsavedChange }: { meeting: any; readOnly: boolean; onUnsavedChange?: (v: boolean) => void }) {
  ...
  const { data: points } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('meeting_discussion_points').select('*').eq('meeting_id', meeting.id).order('sequence');
      return data || [];
    },
  });
  ...
  const addPointMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await DB.from('meeting_discussion_points').insert({
        meeting_id: meeting.id,
        title: newPointTitle,
        sequence: seq,
        created_by: user!.id,
  ...
  const updateNoteMutation = useMutation({
    mutationFn: async ({ pointId, notes }: { pointId: string; notes: string }) => {
      await DB.from('meeting_discussion_points').update({ notes }).eq('id', pointId);
    },
  });
  ...
  const reorderMutation = useMutation({
    mutationFn: async ({ pointId, direction }: { pointId: string; direction: 'up' | 'down' }) => {
      ...
      await Promise.all([
        DB.from('meeting_discussion_points').update({ sequence: points[swapIdx].sequence }).eq('id', points[idx].id),
        DB.from('meeting_discussion_points').update({ sequence: points[idx].sequence }).eq('id', points[swapIdx].id),
      ]);
    },
...
function DecisionsTab({ meeting, readOnly }: { meeting: any; readOnly: boolean }) {
  ...
  const { data: points } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('meeting_discussion_points').select('id, title').eq('meeting_id', meeting.id).order('sequence');
      return data || [];
    },
  });
  ...
  const { data: departments } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('department').select('id, name').eq('is_active', true).order('display_order');
      return data || [];
    },
    enabled: createTask,
  });
  ...
  const { data: deptUsers } = useQuery({
    ...
    queryFn: async () => {
      const { data: uds } = await DB.from('user_departments').select('user_id').eq('department_id', taskDeptId);
      if (!uds?.length) return [];
      const { data } = await DB.from('profiles').select('id, full_name').in('id', uds.map((u) => u.user_id)).eq('is_active', true);
      return data || [];
    },
  ...
  const addDecisionMutation = useMutation({
    mutationFn: async () => {
      ...
      if (createTask) {
        ...
        const { data: taskData, error: taskErr } = await DB.from('tasks').insert({
          title: taskTitle,
      ...
      const { data, error } = await DB.from('meeting_decisions').insert({
        meeting_id: meeting.id,
...
function StandaloneTaskModal({ open, onOpenChange, meetingId }: { open: boolean; onOpenChange: (v: boolean) => void; meetingId?: string }) {
  ...
  const { data: deptUsers } = useQuery({
    ...
    queryFn: async () => {
      const { data: uds } = await DB.from('user_departments').select('user_id').eq('department_id', deptId);
      if (!uds?.length) return [];
      const { data } = await DB.from('profiles').select('id, full_name').in('id', uds.map((u) => u.user_id)).eq('is_active', true);
      return data || [];
    },
