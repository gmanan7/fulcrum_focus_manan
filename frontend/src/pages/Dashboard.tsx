...
export default function Dashboard() {
  ...
  const { data: allDepartments } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('department').select('id, name').eq('is_active', true);
      return data || [];
    },
  });
  ...
  const { data: projectItems } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await DB.from('project_tracker_items').select('*');
      if (error) throw error;
      return data;
    },
  });
  ...
  const { data: kpiTasks } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await DB.from('tasks').select('origin_kpi_entry_id').not('origin_kpi_entry_id', 'is', null);
      if (error) throw error;
      return data;
    },
  });
  ...
  const { data: pmMachines } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await DB.from('pm_machines').select('id, line').eq('is_active', true);
      if (error) throw error;
      return data || [];
    },
  });
  ...
  const { data: pmPlans } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await DB.from('pm_plan').select('machine_id, planned_date')
  ...
  const { data: pmActuals } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await DB.from('pm_actual').select('machine_id, actual_date')
