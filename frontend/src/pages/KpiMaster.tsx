...
function useKpis(deptFilter: string) {
  return useQuery({
    ...
    queryFn: async () => {
      let q = DB.from('kpi_master').select('*, department:department_id(name)').order('display_order');
...
function KpiFormDialog({ initial, departments, onClose }: { initial?: KpiForm; departments: { id: string; name: string; code: string }[]; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit) {
        const { error } = await DB.from('kpi_master').update(payload).eq('id', form.id!);
        if (error) throw error;
        try { await logAudit('kpi_master', form.id!, 'UPDATE', null, payload); } catch (e) { console.warn('Audit log failed:', e); }
      } else {
        const { data: inserted, error } = await DB.from('kpi_master').insert(payload).select('id').single();
        if (error) throw error;
...
export default function KpiMaster() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      // Check for entries
      const { count } = await DB.from('kpi_entries').select('id', { count: 'exact', head: true }).eq('kpi_id', id);
      if (count && count > 0) {
        const deactivate = confirm('This KPI has entries. Deactivate instead of deleting?');
        if (deactivate) {
        const { error } = await DB.from('kpi_master').update({ is_active: false }).eq('id', id);
        if (error) throw error;
      ...
      const { error } = await DB.from('kpi_master').delete().eq('id', id);
      if (error) throw error;
  ...
  const toggleHiddenMutation = useMutation({
    mutationFn: async ({ id, value }: { id: string; value: boolean }) => {
      const { error } = await DB.from('kpi_master').update({ is_hidden_from_trends: value }).eq('id', id);
      if (error) throw error;
    },
