...
export default function Compliance() {
  ...
  const { data: invitees } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await DB.from('meeting_invitees').select('*').in('meeting_id', meetingIds);
      return data || [];
    },
  ...
  const { data: attendance } = useQuery({
    ...
    queryFn: async () => {
      if (!meetingIds.length) return [];
      const { data } = await DB.from('meeting_attendance').select('*').in('meeting_id', meetingIds);
      return data || [];
    },
    enabled: meetingIds.length > 0,
  });
  ...
  const { data: kpiTasks } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('tasks').select('origin_kpi_entry_id').not('origin_kpi_entry_id', 'is', null);
      return data || [];
    },
  });
