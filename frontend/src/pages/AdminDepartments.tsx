...
function useFactory() {
  return useQuery({
    queryKey: ['factory'],
    queryFn: async () => {
      const { data, error } = await DB.from('factory').select('*').limit(1).single();
...
function useDepartments() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await DB.from('department').select('*').order('display_order');
      if (error) throw error;
      return data;
    },
  });
...
function EditFactoryDialog({ factory }: { factory: { id: string; name: string; code: string; location: string | null } }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const { error } = await DB.from('factory').update({ name: form.name, code: form.code, location: form.location || null }).eq('id', factory.id);
      if (error) throw error;
    },
    onSuccess: () => {
...
function AddDeptDialog({ factoryId }: { factoryId: string }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const { error } = await DB.from('department').insert({ factory_id: factoryId, name: form.name, code: form.code, display_order: form.display_order });
      if (error) throw error;
    },
    onSuccess: () => {
...
function useDeptActions() {
  ...
  const deactivateMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await DB.from('department').update({ is_active: false }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const reactivateMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await DB.from('department').update({ is_active: true }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const checkDependencies = async (deptId: string) => {
    const [usersRes, kpisRes, tasksRes] = await Promise.all([
      DB.from('user_departments').select('id', { count: 'exact', head: true }).eq('department_id', deptId),
      DB.from('kpi_master').select('id', { count: 'exact', head: true }).eq('department_id', deptId),
      DB.from('tasks').select('id', { count: 'exact', head: true }).eq('department_id', deptId),
    ]);
    return {
