...
export default function KpiTrends() {
  ...
  const { data: userDepartmentIds } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      const { data } = await DB.from('user_departments').select('department_id').eq('user_id', user.id);
  ...
  const { data: departments } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('department').select('*').eq('is_active', true).order('display_order');
      return data || [];
    },
  });
  ...
  const { data: projectItems } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('project_tracker_items').select('*').order('display_order');
      return data || [];
    },
  });
