...
function CreateMeetingDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const { data: facilitators } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await DB.from('profiles').select('id, full_name').in('id', ids).eq('is_active', true);
      return data || [];
    },
    enabled: open,
  });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      if (templateId !== 'custom' && meeting) {
        const { data: tplInvitees } = await DB
        ...
        if (tplInvitees?.length) {
          ...
          await DB.from('meeting_invitees').insert(inviteeRows);
        }
      }
    },
    onSuccess: () => {
