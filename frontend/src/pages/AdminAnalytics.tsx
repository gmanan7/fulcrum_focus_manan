  ...
  const { data, isLoading, refetch, dataUpdatedAt } = useQuery({
    ...
    queryFn: async () => {
      ...
      const kpiMasterQ = DB
        ...
        .eq('is_active', true);
      const profilesQ = DB.from('profiles').select('id,full_name,is_active');
      const rolesQ = DB.from('user_roles').select('user_id,role');
      const userDeptsQ = DB.from('user_departments').select('user_id,department_id');
      const deptQ = DB.from('department').select('id,name,code,is_active').eq('is_active', true);
      const meetingsQ = DB
        .from('meetings')
        .select('id,title,scheduled_date,status,created_at')
        .order('scheduled_date', { ascending: false });
      const attendanceQ = DB.from('meeting_attendance').select('meeting_id,status');
      const discussionQ = DB.from('meeting_discussion_points').select('meeting_id,notes');
      const decisionsQ = DB.from('meeting_decisions').select('meeting_id');
