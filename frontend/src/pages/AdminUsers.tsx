...
function useUsers(showInactive: boolean) {
  return useQuery({
    ...
    queryFn: async (): Promise<UserRow[]> => {
      const profilesQ = DB.from('profiles').select('*');
      ...
      const { data: allDepts } = await DB.from('user_departments').select('user_id, department_id, department:department(id, name)');
...
function CreateUserDialog() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await DB.functions.invoke('create_user', {
        body: {
          ...form,
...
function EditUserDialog({ user, onClose }: { user: UserRow; onClose: () => void }) {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error: roleErr } = await DB.from('user_roles').insert({ user_id: user.id, role: form.role });
      if (roleErr) throw roleErr;

      // 3. Update departments: delete existing, insert new
      await DB.from('user_departments').delete().eq('user_id', user.id);
...
function ResetPasswordDialog({ user, onClose }: { user: UserRow; onClose: () => void }) {
  ...
  const resetMutation = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await DB.functions.invoke('reset_password', {
...
export default function AdminUsers() {
  ...
  const reactivateMutation = useMutation({
    mutationFn: async (userId: string) => {
      const { error } = await DB.from('profiles').update({ is_active: true }).eq('id', userId);
      if (error) throw error;
    },
    onSuccess: (_, userId) => {
