...
export function ChartFormDialog({ open, onClose, onSaved, chartId, nextDisplayOrder }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit && id) {
        const { error } = await DB.from('kpi_charts').update(chartPayload).eq('id', id);
        if (error) throw error;
      } else {
      ...
      // Replace KPI rows
      const { error: delErr } = await DB.from('kpi_chart_kpis').delete().eq('chart_id', id!);
      ...
      if (form.kpis.length > 0) {
        ...
        const { error: insErr } = await DB.from('kpi_chart_kpis').insert(rows);
