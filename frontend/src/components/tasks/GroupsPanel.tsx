...
function CreateGroupForm({ onCreated, onCancel }: { onCreated: () => void; onCancel: () => void }) {
  ...
  const { data: factory } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await DB.from('factory').select('id').eq('is_active', true).limit(1).single();
      return data;
    },
  });
  ...
  const { data: userDepts } = useQuery({
    ...
    queryFn: async () => {
      const { data: uds } = await DB.from('user_departments').select('user_id, department_id, is_primary');
      const { data: depts } = await DB.from('department').select('id, name');
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      if (memberIds.length > 0) {
        ...
        const { error: e2 } = await DB.from('task_group_members' as any).insert(rows as any);
        if (e2) throw e2;
      }
    },
...
function GroupDetail({
  ...
  const removeMember = useMutation({
    mutationFn: async (rowId: string) => {
      const { error } = await DB.from('task_group_members' as any).delete().eq('id', rowId);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const leave = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await DB.from('task_group_members' as any).delete().eq('id', (myRow as any).id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const del = useMutation({
    mutationFn: async () => {
      const { error } = await DB.from('task_groups' as any).delete().eq('id', groupId);
      if (error) throw error;
    },
    onSuccess: () => {
...
function AddMembersInline({
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await DB.from('task_group_members' as any).insert(rows as any);
      if (error) throw error;
    },
