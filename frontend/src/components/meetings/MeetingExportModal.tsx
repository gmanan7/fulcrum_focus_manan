...
export function MeetingExportModal({ open, onOpenChange, meeting }: MeetingExportModalProps) {
  ...
  const { data: facilitator } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await DB.from('profiles').select('full_name').eq('id', meeting.facilitator_id).maybeSingle();
      return data;
    },
  ...
  const { data: kpiSnapshot } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: kpis }, { data: yEntries }, mtdEntries, { data: depts }] = await Promise.all([
        DB.from('kpi_master').select('*').eq('is_active', true).in('kpi_type', ['numeric', 'descriptive']).order('display_order'),
        DB.from('kpi_entries').select('*').eq('reporting_date', kpiDate),
